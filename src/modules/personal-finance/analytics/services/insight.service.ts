  ...
  private async generateSavingsInsight(
    ...
    if (totalUnderBudget > 0) {
      return {
        ...
        description: `Bạn đã tiết kiệm được ₫${(totalUnderBudget / 100).toLocaleString('vi-VN')}`,
        metric: `Tiết kiệm ₫${(totalUnderBudget / 100).toLocaleString('vi-VN')}`,
        actionable: false,
        priority: 'LOW',
        timestamp: new Date(),
      };
  ...
  private async generateOverBudgetWarning(
    ...
    if (overBudgetCount > 0) {
      return {
        ...
        description: `Tổng vượt ₫${(totalOverage / 100).toLocaleString('vi-VN')}`,
          ...
          suggestedActions: [
            {
              id: uuidv4(),
              title: 'Giảm chi tiêu',
              description: `Cần giảm ₫${((budget.amountLimitCents - spent) / 100).toLocaleString('vi-VN')} để không vượt`,
              impact: 'POSITIVE',
              difficulty: 'MEDIUM',
