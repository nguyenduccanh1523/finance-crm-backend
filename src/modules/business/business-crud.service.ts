...
export class BusinessCrudService {
  ...
  private async membership(orgId: string, userId: string) {
    const membership = await this.dataSource
      ...
      .findOne({ where: { orgId, userId, status: 1 } });
    if (!membership)
  ...
  private resource(name: string) {
    ...
    if (!resource)
      throw new NotFoundException(`Unknown business resource: ${name}`);
    return resource;
  }
  ...
  private safePayload(
    ...
    const protectedFields = new Set([
      ...
      'updatedAt',
      'deletedAt',
      ...(resource.membershipFields || []),
      ...(resource.userFields || []),
    ]);
    const clean = Object.fromEntries(
      Object.entries(payload || {}).filter(
        ([key]) => permitted.has(key) && !protectedFields.has(key),
      ),
    );
    Object.assign(clean, { orgId });
    for (const field of resource.membershipFields || [])
      clean[field] = membership.id;
  ...
  async list(
    ...
    const sortBy = columns.has(query.sortBy || '')
      ? query.sortBy!
      : 'createdAt';
    const qb = repository
      .createQueryBuilder('item')
      .where('item.org_id = :orgId', { orgId });
    if (query.q && resource.searchColumns.length) {
      const searchable = resource.searchColumns.filter((field) =>
        columns.has(field),
      );
      qb.andWhere(
        `(${searchable.map((field) => `CAST(item.${metadata.findColumnWithPropertyName(field)!.databaseName} AS text) ILIKE :q`).join(' OR ')})`,
        { q: `%${query.q.trim()}%` },
      );
    }
    for (const [key, value] of Object.entries(query.filters || {})) {
      ...
      if (column && value !== undefined)
        qb.andWhere(`item.${column.databaseName} = :filter_${key}`, {
          [`filter_${key}`]: value,
        });
    }
    const [data, total] = await qb
      .orderBy(
        `item.${metadata.findColumnWithPropertyName(sortBy)!.databaseName}`,
  ...
  async get(name: string, id: string, orgId: string, userId: string) {
    ...
    const item = await this.dataSource
      .getRepository(this.resource(name).entity)
      .findOne({ where: { id, orgId } as any });
    if (!item) throw new NotFoundException('Resource not found');
    return {
      statusCode: 200,
      message: 'Business resource retrieved',
      data: item,
    };
  }
  ...
  async create(
    ...
    const repository = this.dataSource.getRepository(
      this.resource(name).entity,
    );
    const item = repository.create(
      this.safePayload(name, payload, orgId, membership, userId),
    );
    return {
      statusCode: 201,
      message: 'Business resource created',
      data: await repository.save(item),
    };
  }
  ...
  async update(
    ...
    return {
      statusCode: 200,
      message: 'Business resource updated',
      data: await repository.save(item),
    };
  }
  ...
  async remove(name: string, id: string, orgId: string, userId: string) {
    ...
    return {
      statusCode: 200,
      message: 'Business resource deleted',
      data: null,
    };
  }
}
